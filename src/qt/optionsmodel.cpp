...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeHydraAmount"))
        settings.setValue("nAnonymizeHydraAmount", 1000);
    nAnonymizeHydraAmount = settings.value("nAnonymizeHydraAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeHydraAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeHydraAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeHydraAmount:
            return QVariant(nAnonymizeHydraAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeHydraAmount:
            nAnonymizeHydraAmount = value.toInt();
            settings.setValue("nAnonymizeHydraAmount", nAnonymizeHydraAmount);
            emit AnonymizeHydraAmountChanged(nAnonymizeHydraAmount);
            break;
        default:
            break;
        }

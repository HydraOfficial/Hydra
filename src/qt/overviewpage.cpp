...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeHydraAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeHydraAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeHydraAmount = strAnonymizeHydraAmount.remove(strAnonymizeHydraAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeHydraAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeHydraAmount*COIN) nMaxToAnonymize = nAnonymizeHydraAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeHydraAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeHydraAmount));
        strAnonymizeHydraAmount = strAnonymizeHydraAmount.remove(strAnonymizeHydraAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeHydraAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
